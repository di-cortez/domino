...
- both final hands, final stock, winner, and separate terminal outcome and
  remaining-pip reward components.
...
draw/pass reward events plus the terminal reward components. Each future item
...
Each runtime file contains eight logical tables: final, pips, pass, and draw,
each indexed once by engine-turn distance and once by genuine-neural-decision
...
Final, pass, and draw coefficients use only unit signs: terminal win/opponent
event is `+1`, while terminal loss/neural event is `-1`. Pips coefficients are
nonnegative unit counts of the neural player's remaining terminal pips; every
pip contributes exactly `+1` before later reward scaling. Thus the artifact
contains neither the production pip multiplier (`-0.05`) nor pass/draw reward
magnitudes, gamma, eta, sample counts, smoothing, or interpolation. Runtime
evaluation of any component is simply:

```text
sum(coefficient[k] * gamma**k for k in histogram bins)
```

The future baseline consumer will apply the configured component magnitudes
and combine the resulting terminal and local estimates with `reward_eta`.
That mixing remains deliberately outside these lookup artifacts: changing eta
does not require rebuilding or editing a table.
