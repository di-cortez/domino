...
The final coefficient is in exponent-zero bin: `+1 * gamma**0 = +1` for every
gamma. The empty pips histogram records exactly zero remaining pips. Empty
pass and draw lists are likewise identically zero histograms, not unknown
data.
...
| --- | --- | --- |
| `(1,n)` for any opponent size | Immediate terminal win; final `[1]`, pips/pass/draw `[]` | Exact structural game rule |
...
counts stored dense histogram bins across final, pips, pass, and draw and
...
The recipe is explicit in that script. It applies every operation to final,
pips, pass, and draw under both clocks, so a resolved cell is always reused as
one indivisible set of eight histograms.
