...
class CellAccumulator:
    ...
    def add(self, component, clock, exponent, sign):
        ...
        self.histograms[component][clock][exponent] += sign
    ...
    def add_pips(self, clock, exponent, remaining_pips):
        """Add a nonnegative unit-pip terminal contribution."""
        exponent = int(exponent)
        ...
        if remaining_pips < 0:
            raise ValueError(
                f"Remaining pips cannot be negative, got {remaining_pips}"
            )
        self.histograms["pips"][clock][exponent] += remaining_pips
        self.max_exponents[clock] = max(self.max_exponents[clock], exponent)
        if self.direct_totals is not None:
            for gamma in VALIDATION_GAMMAS:
                self.direct_totals["pips"][clock][gamma] += (
                    remaining_pips * gamma ** exponent
                )
...
def _terminal_contribution(sample):
    ...
    outcome_reward = float(terminal.get("outcome_reward"))
    if not math.isfinite(outcome_reward) or outcome_reward * sign <= 0.0:
        raise ValueError("Terminal outcome reward sign disagrees with result")
    remaining_pips = terminal.get("remaining_pips")
    if isinstance(remaining_pips, bool) or not isinstance(remaining_pips, int):
        raise ValueError(
            f"Terminal remaining pips must be an integer, got {remaining_pips!r}"
        )
    if remaining_pips < 0:
        raise ValueError("Terminal remaining pips cannot be negative")
    exponents = {
    ...
    return sign, remaining_pips, exponents
...
def _local_contributions(sample):
    ...
    for event in events:
        ...
        base_reward = float(event.get("base_reward"))
        if not math.isfinite(base_reward) or base_reward * sign <= 0.0:
...
def accumulate_sample(accumulator, sample):
    """Add one decision's unit terminal, pips, and local-event suffix."""
    if not isinstance(sample, dict):
        raise ValueError("Lookup sample must be an object")
    terminal_sign, remaining_pips, terminal_exponents = _terminal_contribution(sample)
    ...
    for clock, exponent in terminal_exponents.items():
        accumulator.add("final", clock, exponent, terminal_sign)
        accumulator.add_pips(clock, exponent, remaining_pips)
...
def _validate_cell(accumulator, expected_count, cell_key):
    ...
    expected_final_sum = accumulator.wins - accumulator.losses
    for clock in CLOCKS:
        actual = sum(accumulator.histograms["final"][clock].values())
        if actual != expected_final_sum:
            raise ValueError(f"Cell {cell_key} final {clock} sum is inconsistent")
        if any(
            value < 0
            for value in accumulator.histograms["pips"][clock].values()
        ):
            raise ValueError(f"Cell {cell_key} pips {clock} contains a negative bin")
    for component in COMPONENTS:
        turn_sum = sum(accumulator.histograms[component]["turn"].values())
        decision_sum = sum(
            accumulator.histograms[component]["decision"].values()
        )
        if turn_sum != decision_sum:
            raise ValueError(
...
def _normalized_cell(accumulator, cell_key):
    ...
    for clock in CLOCKS:
        final_sum = sum(normalized["final"][clock])
        if not -1.0 - 1e-12 <= final_sum <= 1.0 + 1e-12:
            raise ValueError(f"Cell {cell_key} final histogram is outside [-1, 1]")
    for component in COMPONENTS:
        if not math.isclose(
