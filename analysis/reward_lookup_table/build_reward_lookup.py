...
def _future_local_events(turns, decision_turn, decision_index):
    ...
    for turn in turns:
        ...
        events.append({
            "kind": event["kind"],
            "actor": event["actor"],
            "base_reward": float(event["base_reward"]),
...
def _sample_from_decision(record, turn):
    ...
    sample = {
        ...
        "future_local_reward_sum_undiscounted": float(
            sum(event["base_reward"] for event in local_events)
        ),
        "terminal": {
            "outcome_reward": float(terminal_component["outcome_reward"]),
            "remaining_pips": int(terminal_component["remaining_pips"]),
            "remaining_pip_penalty": float(
                terminal_component["remaining_pip_penalty"]
            ),
            "base_reward": float(terminal_component["base_reward"]),
    ...
    with tempfile.TemporaryDirectory(
        ...
        header = {
            ...
            "sample_semantics": (
                ...
                "later draw/pass rewards, and terminal components. No gamma_i, "
            ...
            "reward_schema": raw_manifest["configuration"]["reward_schema"],
