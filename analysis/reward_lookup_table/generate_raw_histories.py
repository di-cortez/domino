...
from reward_lookup_common import (
    ...
    serialize_action,
    terminal_components_for_neural,
    tile_option_count,
...
def _worker_initializer(weights_path, ruleset_name, base_seed):
    ...
    global _WORKER_NEURAL, _WORKER_HEURISTIC
...
def _play_game(game_index):
    ...
    while not engine.game_over:
        ...
        if event is not None:
            local_event_sum += float(event["unit_reward"])
    ...
    terminal = terminal_components_for_neural(final_state, neural_seat)
    ...
    record = {
        ...
        "raw_rewards_for_neural": {
            "local_event_unit_sum": float(local_event_sum),
            "terminal": terminal,
        },
    }
    line = json.dumps(
...
def _configuration(args, ruleset_name, checkpoint):
    ...
    return {
        ...
        "reward_semantics": raw_reward_semantics(),
        "neural_policy": {
            key: value
