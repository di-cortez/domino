...
|       Arquivo       |                           Conteúdo                            |
|---------------------|---------------------------------------------------------------|
|`pesos_domino_sl.npz`|Pesos da `RedeNeuralSupervisionada` (79→256→128→58) treinada   |
|                     |por imitação do heurístico. Gerado por `training_loop.py`;     |
|                     |consumido por `AgenteNeuralNumPy` (opção `neural` no menu).    |
---------------------------------------------------------------------------------------
|`pesos_domino_rl.npz`|Pesos da `RedeNeuralPolitica` (mesma arquitetura), refinados   |
|                     |por self-play a partir do SL. Gerado por `self_play.py`;       |
|                     |consumido por `AgenteRL` (opção `rl` no menu, sempre greedy).  |
---------------------------------------------------------------------------------------
