...
class ParallelDiagnosticsTests(unittest.TestCase):
    ...
    def test_all_pairs_autotunes_each_matchup_independently(self):
        ...
        self.assertEqual(
            set(report["selected_workers_by_matchup"]),
            {"rl_vs_random", "heuristic_vs_random"},
        )
        self.assertEqual(set(report["autotune"]["matchups"]), {
            "rl_vs_random",
            "heuristic_vs_random",
        })
        self.assertEqual(report["autotune"]["reused_game_count"], 8)
        for tuning in report["autotune"]["matchups"].values():
            self.assertEqual(tuning["games_per_test"], 2)
            self.assertEqual(tuning["reused_game_count"], 4)
