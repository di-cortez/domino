...
# The canonical supervised dataset is generated by StrategicAgent against
# StrategicAgent, so the seat being imitated was facing the heuristic. Encoding
# every example with that bucket keeps the pretraining distribution inside the
# one RL will reproduce, instead of an all-zero pattern RL never shows again.
SUPERVISED_TEACHER_OPPONENT_BUCKET = "heuristic"
...
def train_supervised(
    ...
    ``use_opponent_bucket_features=True`` lengthens both instead, so the
    supervised weights stay loadable by an RL run that appends the block. The
    canonical dataset is ``StrategicAgent`` against ``StrategicAgent``, so every
    example is encoded with the ``heuristic`` bucket: that is genuinely who the
    seat being imitated was playing, and it is a bucket the RL rollout uses too,
    so pretraining never teaches the network a pattern RL will not show again.
    """
    if epochs < 1:
    ...
    encoder = DominoEncoder(
        ...
        use_opponent_suit_features=use_opponent_suit_features,
        use_opponent_bucket_features=use_opponent_bucket_features,
        opponent_bucket=SUPERVISED_TEACHER_OPPONENT_BUCKET,
    )
